...
//Article Numbers
if (
    localStorage.getItem("numArticles") === null ||
    localStorage.getItem("numArticles") === 0 ||
    localStorage.getItem("numArticles") === "NaN" ||
    localStorage.getItem("numArticles") === NaN
) {
    let numArticles = 0;
    localStorage.setItem("numArticles", numArticles);
    UInumArticles.appendChild(
        document.createTextNode(localStorage.getItem("numArticles"))
    );
} else {
    let UInumArticles = JSON.parse(localStorage.getItem("numArticles"));
    let arrArticles = [];
    for (i = 0; i <= numArticles; i++) {
        UIview.innerHTML = `${JSON.parse(
            localStorage.getItem(`numArticles ${i}`)
        )}`;
    }
}
...
function postArticle(e) {
    ...
    numArticles = parseInt(numArticles) + 1;
...
function createArticle() {
    ...
    img.setAttribute(
        "src",
        `https://source.unsplash.com/random/600x450?${valueSubject}&sig=${IDgenerateMoreImages}`
    );
...
//Used to Prepare the app at startup or reset it
function reset() {
    //delete all articles
